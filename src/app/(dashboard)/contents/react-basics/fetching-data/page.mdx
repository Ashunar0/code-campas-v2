
> 📌 **この記事で学ぶこと**
>
> - useEffect と fetch の組み合わせ
> - ローディング状態の管理
> - エラーハンドリング
> - 実践例

## React での API 取得パターン

React で API からデータを取得する基本パターンは以下の通りです。

```jsx
import { useState, useEffect } from "react";

function App() {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("https://api.example.com/data");
        const result = await response.json();
        setData(result);
      } catch (err) {
        setError(err.message);
      } finally {
        setLoading(false);
      }
    };

    fetchData();
  }, []);

  if (loading) return <p>読み込み中...</p>;
  if (error) return <p>エラー: {error}</p>;
  if (!data) return <p>データがありません</p>;

  return <div>{/* データを表示 */}</div>;
}
```

## 3 つの State

API 取得では、3 つの State を管理するのが一般的です。

| State     | 役割               |
| --------- | ------------------ |
| `data`    | 取得したデータ     |
| `loading` | 読み込み中かどうか |
| `error`   | エラー情報         |

## 実践例：PokeAPI

```jsx
import { useState, useEffect } from "react";

function PokemonCard({ id }) {
  const [pokemon, setPokemon] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fetchPokemon = async () => {
      try {
        setLoading(true);
        const response = await fetch(`https://pokeapi.co/api/v2/pokemon/${id}`);

        if (!response.ok) {
          throw new Error("ポケモンが見つかりません");
        }

        const data = await response.json();
        setPokemon(data);
      } catch (err) {
        setError(err.message);
      } finally {
        setLoading(false);
      }
    };

    fetchPokemon();
  }, [id]); // id が変わったら再取得

  if (loading) {
    return <p>読み込み中...</p>;
  }

  if (error) {
    return <p>エラー: {error}</p>;
  }

  return (
    <div className="pokemon-card">
      <img src={pokemon.sprites.front_default} alt={pokemon.name} />
      <h2>{pokemon.name}</h2>
      <p>No. {pokemon.id}</p>
      <p>高さ: {pokemon.height / 10}m</p>
      <p>重さ: {pokemon.weight / 10}kg</p>
    </div>
  );
}

export default PokemonCard;
```

## ローディング表示

ユーザーに「読み込み中」であることを伝えましょう。

```jsx
if (loading) {
  return (
    <div className="loading">
      <div className="spinner"></div>
      <p>読み込み中...</p>
    </div>
  );
}
```

### CSS でスピナーを作る

```css
.spinner {
  width: 40px;
  height: 40px;
  border: 4px solid #f3f3f3;
  border-top: 4px solid #3498db;
  border-radius: 50%;
  animation: spin 1s linear infinite;
}

@keyframes spin {
  0% {
    transform: rotate(0deg);
  }
  100% {
    transform: rotate(360deg);
  }
}
```

## エラーハンドリング

API 取得は失敗する可能性があります。

```jsx
useEffect(() => {
  const fetchData = async () => {
    try {
      const response = await fetch(url);

      // レスポンスが正常かチェック
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }

      const data = await response.json();
      setData(data);
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
    }
  };

  fetchData();
}, []);
```

## 実践例：ポケモンリスト

```jsx
import { useState, useEffect } from "react";

function PokemonList() {
  const [pokemons, setPokemons] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchPokemons = async () => {
      try {
        const response = await fetch(
          "https://pokeapi.co/api/v2/pokemon?limit=20"
        );
        const data = await response.json();
        setPokemons(data.results);
      } catch (err) {
        console.error("エラー:", err);
      } finally {
        setLoading(false);
      }
    };

    fetchPokemons();
  }, []);

  if (loading) {
    return <p>読み込み中...</p>;
  }

  return (
    <ul>
      {pokemons.map((pokemon, index) => (
        <li key={pokemon.name}>
          #{index + 1} {pokemon.name}
        </li>
      ))}
    </ul>
  );
}
```

## 検索機能の実装

```jsx
function PokemonSearch() {
  const [searchName, setSearchName] = useState("");
  const [pokemon, setPokemon] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  const handleSearch = async () => {
    if (!searchName.trim()) return;

    setLoading(true);
    setError(null);
    setPokemon(null);

    try {
      const response = await fetch(
        `https://pokeapi.co/api/v2/pokemon/${searchName.toLowerCase()}`
      );

      if (!response.ok) {
        throw new Error("ポケモンが見つかりません");
      }

      const data = await response.json();
      setPokemon(data);
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
    }
  };

  return (
    <div>
      <input
        type="text"
        value={searchName}
        onChange={(e) => setSearchName(e.target.value)}
        placeholder="ポケモン名を入力"
      />
      <button onClick={handleSearch} disabled={loading}>
        検索
      </button>

      {loading && <p>検索中...</p>}
      {error && <p>エラー: {error}</p>}
      {pokemon && (
        <div>
          <h2>{pokemon.name}</h2>
          <img src={pokemon.sprites.front_default} alt={pokemon.name} />
        </div>
      )}
    </div>
  );
}
```

## まとめ

- **useEffect** + **fetch** で API からデータを取得
- 3 つの State：**data**, **loading**, **error**
- **依存配列**で再取得のタイミングを制御
- **try-catch-finally** でエラーハンドリング
- ローディング表示でユーザー体験を向上

---

## 🔗 次のステップ

API 取得がわかったら、State のリフトアップ に進みましょう！
